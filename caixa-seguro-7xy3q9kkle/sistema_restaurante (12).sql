...
-- Procedimentos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `devolver_item_comanda` (IN `p_item_id` INT, IN `p_quantidade` INT, IN `p_observacao` TEXT, IN `p_usuario_id` INT)   BEGIN    
    ...
    IF p_quantidade > v_quantidade_original THEN        
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Quantidade de devolução maior que a quantidade original';    
    END IF;        
    ...
    -- Registrar devolução no estoque (sem valor_unitario)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `limpar_dados_teste` ()   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        RESIGNAL;
...
`gorjetas_hoje` decimal(32,2)
,`gorjetas_semana` decimal(32,2)
);
...
--
-- Estrutura stand-in para view `view_performance_garcons`
-- (Veja abaixo para a visão atual)
--
CREATE TABLE `view_performance_garcons` (
`id` int(11)
,`nome` varchar(100)
,`codigo` varchar(10)
,`total_comandas` bigint(21)
...
,`ticket_medio` decimal(14,6)
,`total_gorjetas_geradas` decimal(32,2)
,`dias_trabalhados` bigint(21)
,`media_diaria` decimal(36,6)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_dashboard_aprimorado`  AS SELECT (select coalesce(sum(`comandas`.`taxa_gorjeta`),0) from `comandas` where cast(`comandas`.`data_venda` as date) = curdate() and `comandas`.`status` = 'fechada') AS `gorjetas_hoje`, (select coalesce(sum(`comandas`.`taxa_gorjeta`),0) from `comandas` where yearweek(`comandas`.`data_venda`,0) = yearweek(current_timestamp(),0) and `comandas`.`status` = 'fechada') AS `gorjetas_semana` ;
...
--
-- Estrutura para view `view_performance_garcons`
--
DROP TABLE IF EXISTS `view_performance_garcons`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_performance_garcons`  AS SELECT `g`.`id` AS `id`, `g`.`nome` AS `nome`, `g`.`codigo` AS `codigo`, count(distinct `c`.`id`) AS `total_comandas`, sum(`c`.`valor_total`) AS `valor_total_vendido`, avg(`c`.`valor_total`) AS `ticket_medio`, sum(`c`.`taxa_gorjeta`) AS `total_gorjetas_geradas`, count(distinct cast(`c`.`data_venda` as date)) AS `dias_trabalhados`, sum(`c`.`valor_total`) / count(distinct cast(`c`.`data_venda` as date)) AS `media_diaria` FROM (`garcons` `g` left join `comandas` `c` on(`g`.`id` = `c`.`garcom_id` and `c`.`status` = 'fechada')) WHERE `g`.`ativo` = 1 GROUP BY `g`.`id`, `g`.`nome`, `g`.`codigo` ORDER BY sum(`c`.`valor_total`) DESC ;
